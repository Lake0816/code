...
void *GB_uart_recv_thread(void * pParam)
{
    ...
    pstMsg->lDstID = SKL_MODULE_PRI_UART_GB;
    pstMsg->usCmdID = 0xFFFF;
    
    while (1)
    {
        ulRet = SklUart_recv(giUartFd, 1000, pstMsg->aucData, &recvLen);
        if ((ulRet != SKL_STAT_SUCCESS) || (recvLen <= 0) || (recvLen > 1000))
        {
            continue;
        }

        pstMsg->usLen = recvLen;
        ulRet = SklQueue_snd(pstMsg, SKL_MSG_BLOCK);
        if (ulRet != SKL_STAT_SUCCESS)
        {
            printf("[%s %d] SklQueue_snd ulRet [%lu]\n", __FUNCTION__, __LINE__, ulRet);
        }
...
void GB_uart_sendRspErr(IN UCHAR ucCmd)
{
    ...
    SklUart_send(giUartFd, aucErrPacket, 5);
 ...
 * Date:                      2020.05.21
***********************************************************/
ULONG GB_uart_packet_bcc_check(IN SKL_QUEUE_MSG_s *pstMsg)
{
    ...
    if ((pstMsg->usLen < 7) || (pcData[0] != 0xAA) || (pcData[1] != 0x75))
    {
        printf("[%s %d] recv err packet [%s] len %d\n", __FUNCTION__, __LINE__, 
               pcData, pstMsg->usLen);
        return SKL_STAT_FAIL_GB_BEGIN_FLAG;
    }
    ...
    if (pcData[pstMsg->usLen - 1] != ucBCC)
    {
        ...
        return SKL_STAT_FAIL_GB_BCC_CHECK;
    }
...
ULONG GB_uart_TimeFormatBCD(OUT UCHAR *buff)
{
    ...
    {
        perror("time");
        return SKL_STAT_FAIL_GET_TIME;    
    }
    
    if (localtime_r(&cur_time, &local_tm) == NULL)
    {
        perror("localtime" );
        return SKL_STAT_FAIL_GET_LOCALTIME;
    }
...
struct tm{
    ...
    SklAccess_String2Bcd(szTmp + 2, buff, NULL);
...
ULONG GB_uart_sendRspRealTime(void)
{
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + GB_UART_LEN_TIME + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_sendRspStandardVer(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR send_buf[GB_UART_LEN_COMMON + GB_UART_LEN_VER + 1] = 
        {0x55, 0x7A, GB_UART_CLLC_STANDARD_VER};
    ULONG ulRet = SKL_STAT_SUCCESS;
    SKL_STOR_RECORDER_STANDARD_VER_s *pstVer = 
        (SKL_STOR_RECORDER_STANDARD_VER_s *)pstMsg->aucData;

    if (pstMsg->ulRet != SKL_STAT_SUCCESS)
    {
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + GB_UART_LEN_VER + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_sendRspDriverLincese(IN SKL_QUEUE_MSG_s *pstMsg)
{
    ...
    SKL_STOR_DRIVER_LINCESE_s *pstDriverLincese = 
        (SKL_STOR_DRIVER_LINCESE_s *)pstMsg->aucData;
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + GB_UART_LEN_DRIVER_LINCESE + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_sendRspVehicleInfo(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR send_buf[GB_UART_LEN_COMMON + GB_UART_LEN_VEHICLE_INFO + 1] = 
        {0x55, 0x7A, GB_UART_CLLC_VEHICLE_INFO};
    ULONG ulRet = SKL_STAT_SUCCESS;
    SKL_STOR_VEHICLE_INFO_s *pstVehicleInfo = (SKL_STOR_VEHICLE_INFO_s *)pstMsg->aucData;
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + GB_UART_LEN_VEHICLE_INFO + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_sendRspRecordUnique(IN SKL_QUEUE_MSG_s *pstMsg)
{
    ...
    SKL_STOR_RECORDER_UNIQUE_INFO_s *pstRecordUnique = 
        (SKL_STOR_RECORDER_UNIQUE_INFO_s *)pstMsg->aucData;
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + GB_UART_LEN_RECORD_UNIQUE + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_sendRspSet(IN SKL_QUEUE_MSG_s *pstMsg)
{
    ...
    ulRet = SklUart_send(giUartFd, send_buf, GB_UART_LEN_COMMON + 1);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_modulePacketProcess(IN SKL_QUEUE_MSG_s *pstMsg)
{
...
ULONG GB_uart_sendReqToModule(IN UCHAR *reqMsg, IN ULONG ulDstModule)
{
    UCHAR aucBuff[sizeof(SKL_QUEUE_MSG_s) + 1000] = {0};
    ULONG ulRet = SKL_STAT_SUCCESS;
    SKL_QUEUE_MSG_s *pstModuleMsg = (SKL_QUEUE_MSG_s *)aucBuff;
    ...
    pstModuleMsg->lSrcID = SKL_MODULE_ACCESS;

    ulRet = SklQueue_snd(pstModuleMsg, SKL_MSG_BLOCK);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] Queue_snd : Ret %lu\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_getCmd(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR *pcData = pstMsg->aucData;
    ...
    switch (pcData[GB_UART_INDEX_CMD])
    {
        ...
        case GB_UART_CLLC_VEHICLE_INFO:                 /* 车辆信息 */
        case GB_UART_CLLC_RECORDER_UNIQUE_NUMBER:       /* 记录仪唯一编号 */
            GB_uart_sendReqToModule(pcData, SKL_MODULE_STOR);
            break;
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_setVehicleInfo(IN SKL_QUEUE_MSG_s *pstMsg)
{

    UCHAR *pcSrcMsg = pstMsg->aucData;
    UCHAR aucMsg[sizeof(SKL_QUEUE_MSG_s) + sizeof(SKL_STOR_VEHICLE_INFO_s)] = {0};
    USHORT usLen = 0; 
    ULONG ulRet = SKL_STAT_SUCCESS;
    SKL_QUEUE_MSG_s *pstDstMsg = (SKL_QUEUE_MSG_s *)aucMsg;
    SKL_STOR_VEHICLE_INFO_s *pstVehicleInfo = (SKL_STOR_VEHICLE_INFO_s *)pstDstMsg->aucData;

    usLen = SklAccess_String2Short(pcSrcMsg + GB_UART_INDEX_LEN);
    ...
    pstDstMsg->lDstID = SKL_MODULE_STOR;
    pstDstMsg->lSrcID = SKL_MODULE_ACCESS;
    ...
    ulRet = SklQueue_snd(pstDstMsg, SKL_MSG_BLOCK);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d] SklQueue_snd ulRet [%lu]\n", __FUNCTION__, __LINE__, ulRet);
    }
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_setInitialMileage(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR *pcSrcMsg = pstMsg->aucData;
    UCHAR aucMsg[sizeof(SKL_QUEUE_MSG_s) + sizeof(SKL_STOR_ACCUMULATED_MILEAGE_s)] = {0};
    USHORT usLen = SklAccess_String2Short(pcSrcMsg + GB_UART_INDEX_LEN);
    ULONG ulRet = SKL_STAT_SUCCESS;
    SKL_QUEUE_MSG_s *pstDstMsg = (SKL_QUEUE_MSG_s *)aucMsg;
    SKL_STOR_ACCUMULATED_MILEAGE_s *pstAccInfo = 
        (SKL_STOR_ACCUMULATED_MILEAGE_s *)pstDstMsg->aucData;
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_setCmd(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR *pcData = pstMsg->aucData;
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_verification(SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR aucBuff[sizeof(SKL_QUEUE_MSG_s) + 1000] = {0};
    SKL_QUEUE_MSG_s *pstModuleMsg = (SKL_QUEUE_MSG_s *)aucBuff;
    ...
    pstModuleMsg->lDstID = SKL_MODULE_STOR;
    pstModuleMsg->lSrcID = SKL_MODULE_ACCESS;
}
 ...
 * Date:                      2020.05.21
***********************************************************/
void GB_uart_main_process(IN SKL_QUEUE_MSG_s *pstMsg)
{
    UCHAR *pcData = pstMsg->aucData;
...
void *GB_uart_parse_thread(void * pParam)
{
    ...
    while (1)
    {
        ...
        {
            /* uart recv packet */
            if (GB_uart_packet_bcc_check(pstMsg) != SKL_STAT_SUCCESS)
            {
                continue;
            }
...
ULONG GB_uart_init(void)
{
    ...
    SKL_UART_CFG_PARAM_s stUartCom;
    
    /* init uart port */
    memset(&stUartCom, 0, sizeof(stUartCom));
    stUartCom.pcName = SKL_VEHICLE_GB_UART;
    stUartCom.baud_rate = 115200;
    stUartCom.flow_ctrl = 0;;
    stUartCom.data_bits = 8;                                                     
	stUartCom.parity    = 'N';                                                      
	stUartCom.stop_bits = 1; 
    ulRet = SklUart_open(stUartCom, &giUartFd);
    if (ulRet != SKL_STAT_SUCCESS)
    {
        printf("[%s %d ] Uart_open %s fail err: %lu\n", __FUNCTION__, __LINE__, 
               SKL_VEHICLE_GB_UART, ulRet);
        return ulRet;
    }
    
    /* register queue */
    ulRet = SklQueue_modRegist(SKL_MODULE_PRI_UART_GB);
    if (ulRet != SKL_STAT_SUCCESS)
    {
...
void GB_uart_destory(void)
{
    ...
    /* unregister queue */
    ulRet = SklQueue_modUnregist(SKL_MODULE_PRI_UART_GB);
    if (ulRet != SKL_STAT_SUCCESS)
    {
